...
function LineUsersPerDay({ metrics, searchParams }) {
  ...
  if (metrics.visitorsByDate && Object.values(metrics.visitorsByDate).length > 0) {
    ...
    while (start.isSameOrBefore(moment(end), 'day')) {
      ...
      const value = metrics.visitorsByDate[start.format('MM-DD')]
        ? Object.values(metrics.visitorsByDate[start.format('MM-DD')]).length
        : 0;
      dataSet.push(value);
